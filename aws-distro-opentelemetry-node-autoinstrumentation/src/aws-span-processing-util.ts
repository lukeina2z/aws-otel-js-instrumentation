...
export class AwsSpanProcessingUtil {
  ...
  private static isSqsReceiveMessageConsumerSpan(spanData: ReadableSpan): boolean {
    ...
    const instrumentationScope: InstrumentationScope = spanData.instrumentationScope;
  ...
  static setIsLocalRootInformation(span: Span, parentContext: Context): void {
    ...
    const isLocalRoot: boolean =
      (span as any).parentSpanContext === undefined || !isParentSpanContextValid || isParentSpanRemote;
